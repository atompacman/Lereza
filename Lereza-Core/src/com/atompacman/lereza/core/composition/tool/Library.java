...
public class Library implements LibraryAPI {
	...
	public void addComposition(String filePath, String compositionName, 
			String composerName, Form musicalForm, String compositionSetName) {
		if (Log.infos() && Log.title("Library", 0));

		Composer composer = Composer.get(composerName);

		if (byComposer.get(composer) != null && 
				byComposer.get(composer).get(compositionName) != null) {
			if (Log.error() && Log.print("Cannot add the composition \"" + compositionName 
					+ "\" in the library as it already contains one with that name.")){}
			return;
		}
		...
		if (byComposer.get(composer).get(compositionSetName) == null) {
			CompositionSet newSet = new CompositionSet(compositionSetName, composer);
			byComposer.get(composer).put(compositionSetName, newSet);
		}
		CompositionSet set = byComposer.get(composer).get(compositionSetName);
		Context context = new Context(musicalForm);
		Composition composition = new Composition(compositionName, set, context, 
				PieceBuilder.getPiece(filePath), MidiFileReader.getMidiFile(filePath));
		set.add(composition);
		...
		if (byContext.get(context).get(compositionName) != null) {
			if (Log.error() && Log.print("Replacing the older composition named \"" 
					+ compositionName + "\" for context \"" + context.toString() + "\"."));
		}
	...
	public static Composition getComposition(String compositionName, 
			String composerName, String compositionSetName) {
		Composer composer = Composer.get(composerName);
		if (byComposer.get(composer) == null) {
			throw new IllegalArgumentException("No such composer as \"" 
					+ composerName + "\" in the library.");
		}
		if (byComposer.get(composer).get(compositionSetName) == null) {
			throw new IllegalArgumentException("No such song as \"" + compositionName 
					+ "\" from composer \"" + composerName + "\" in the library.");
		}
