...
public class CompositionSet {
	...
	public void add(Composition composition) {
		if (compositions.get(composition.getTitle()) != null) {
			if (Log.error() && Log.print("Composition \"" + composition.getTitle() + "\" cannot be add to set " + setName + " as it already contains one with this name."));
		}
		compositions.put(composition.getTitle(), composition);
		if (Log.infos() && Log.print("Composition \"" + composition.getTitle() + "\" add to set " + setName + "."));
	}
	...
	public Composition get(String name) {
		if (compositions.get(name) == null) {
			if (Log.error() && Log.print("No composition named \"" + name + "\" was found in set " + setName + "."));
		}
