...
import com.atompacman.lereza.core.piece.NoteStack.NoteStatus;
...
public class NoteStackBuilder extends PieceComponentBuilder<NoteStack> {
    ...
    private List<Note> startingNotes;
    private List<Note> startedNotes;
    ...
    public NoteStackBuilder add(Note note, int velocity, boolean isStarting) {
        if (isStarting) {
    ...
    public NoteStackBuilder add(Pitch pitch, Value value, int velocity, boolean isStarting) {
        return add(Note.valueOf(pitch, value), velocity, isStarting);
    }

    public NoteStackBuilder add(Note note) {
        return add(note, currVelocity, currIsStarting);
    }

    public NoteStackBuilder add(Pitch pitch) {
        return add(Note.valueOf(pitch, currValue), currVelocity, currIsStarting);
    }

    public NoteStackBuilder add(String pitch) {
        return add(Note.valueOf(pitch, currValue), currVelocity, currIsStarting);
    }

    public NoteStackBuilder value(Value value) {
        currValue = value;
        return this;
    }

    public NoteStackBuilder velocity(int velocity) {
        this.currVelocity = velocity;
        return this;
    }

    public NoteStackBuilder isStarting(boolean isStarting) {
    ...
    protected NoteStack buildComponent() {
        // Create notes by status structure
        Map<NoteStatus, Set<Note>> notesByStatus = createEmptyNoteByStatusStructure();
        ...
        try {
            return new NoteStack(notesByStatus, dynamic);
        } catch (IllegalArgumentException e) {
            signal(Anomaly.MULTIPLE_NOTE_ENTRY_FOR_SAME_PITCH);
        }

        return new NoteStack(createEmptyNoteByStatusStructure(), null);
    }
    
    private static Map<NoteStatus, Set<Note>> createEmptyNoteByStatusStructure() {
        Map<NoteStatus, Set<Note>> notesByStatus = new EnumMap<>(NoteStatus.class);
