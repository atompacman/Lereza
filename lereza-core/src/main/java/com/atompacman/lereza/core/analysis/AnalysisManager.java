...
import com.atompacman.lereza.core.analysis.profile.Study;
...
public class AnalysisManager extends Module {
    ...
    public Study analyze(Piece piece) {
        study = new Study();
            ...
            case PART:
                ...
                for (int i = 0; i < piece.numParts(); ++i) {
                    ...
                    while (!navig.endOfPart()) {
                        navig.goToNextNote();
                        NoteStack stack = navig.getNoteStack();
                        for (Note note : stack.getStartingNotes()) {
                            analyzer.analyzeNote(note, null);
                        }
                    }
