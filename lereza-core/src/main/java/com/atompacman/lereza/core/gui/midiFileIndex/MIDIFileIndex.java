...
public class MIDIFileIndex {
	...
	private static JPanel createUpperPanel() {
		...
		button.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent event) {
				JFileChooser chooser = new JFileChooser(Parameters.Paths.MIDI_ROOT_DIR);
				chooser.setFileFilter(new FileNameExtensionFilter("MIDI files (.mid)", "mid"));
				if(chooser.showOpenDialog(null) == JFileChooser.APPROVE_OPTION) {
					String selectedFile = chooser.getSelectedFile().getAbsolutePath();
					String toRemove = new File(Parameters.Paths.MIDI_ROOT_DIR).getAbsolutePath();
	...
	private static ResultSet queryDB(String sqlFile, boolean storeQuery, String...jokers) {
		Database db = Wizard.getModule(Database.class);
		String query;
		try {
			query = FileUtils.readFileToString(new File(sqlFile));
		} catch (IOException e) {
			throw new RuntimeException("Could not read SQL query "
					+ "template at \"" + sqlFile + "\"", e);
		}
		if (storeQuery) {
