...
public class Log {
	...
	public static boolean print(String message) {
		System.out.println(timestamp() + " " + className() + " {" 
				+ calledVerbose.name() + "} : " + message);
		return true;
	}

	public static boolean title(String title, int spacesBetweenDots) {
		System.out.println(timestamp() + " " + className() + " {" + calledVerbose.name() 
				+ "} : " + generateTitle(title, spacesBetweenDots));
		return true;
	}

	public static boolean title(int spacesBetweenDots) {
		System.out.println(timestamp() + " " + className() + " {" + calledVerbose.name() 
				+ "} : " + generateTitle("", spacesBetweenDots));
		return true;
	}
	...
	private static String className() {
		...
		String className = fullStackTraceElement.substring(fullStackTraceElement.indexOf('('), 
				fullStackTraceElement.indexOf(')') + 1);
