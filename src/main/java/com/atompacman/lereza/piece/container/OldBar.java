...
public class OldBar {
	...
	public OldBar(RythmicSignature rythmicSignature, int barNo) {
		this.notes = new ArrayList<Set<Note>>();
		for (int i = 0; i < rythmicSignature.getBarTimeunitLength(); ++i) {
	...
	public void addNote(MIDINote note, Value value, int timeunit, boolean noteIsTied) {
		...
		if (Log.extra() && Log.print(String.format("Adding note %4s of length "
				+ "%2d at timeunit %4d of bar no.%d", 
				firstNote.toString(), firstNote.getValue().toTimeunit(), timeunit, barNo)));		
	...
	public List<String> toStringList(boolean completedBar) {
		...
		for (int i = 0; i < notes.size(); ++i) {
			...
			if (Parameters.SHOW_RESTS && timeunitNotes.isEmpty() && 
					(areOtherNotesToPlaceFrom(i) || completedBar)) {
	...
	private List<String> createEmptyPartition() {
		...
		toNumber = String.format(" %-4d", barNo) + toNumber.substring(5);
	...
	private String createEmptyLine() {
		int lineLength = rythmicSignature.getBarTimeunitLength() * 
